...
class _Setting_page extends State<Setting_page> {
  ...
  void refreshData() {
    ...
    for (int i = 0; i < 3; i++) {
      controllersIP[i].text =
          settingsJsonData["CPU" + (i + 1).toString() + "_IP"];
      cpuVal[i] = settingsJsonData["CPU" + (i + 1).toString() + "_VAR"];
    }
  }
  ...
  Row devMode() {
    return Row(children: [
      const Text("dev (unstable version):   ",
          style: TextStyle(color: Colors.white, fontSize: 16)),
      Switch(
        value: SettingEventMainData.devModeState,
        activeColor: const Color.fromARGB(255, 67, 102, 255),
        onChanged: (bool value) {
          setState(() {
            SettingEventMainData.devModeState = value;
          });
        },
      )
    ]);
  ...
  Row precisionMode() {
    return Row(children: [
      const Text("precision voltage meter: ",
          style: TextStyle(color: Colors.white, fontSize: 16)),
      Switch(
        value: SettingEventMainData.precisionMeter,
        activeColor: const Color.fromARGB(255, 67, 102, 255),
        onChanged: (bool value) {
          setState(() {
            SettingEventMainData.precisionMeter = value;
            sockEvent.write("PVM_" +
                (SettingEventMainData.precisionMeter ? 1 : 0).toString());
          });
        },
      )
    ]);
      ...
      Container(
          ...
          child: DropdownButtonHideUnderline(
            ...
            onChanged: (String? newValue) {
              setState(() {
                cpuVal[indx] = newValue;
              });
            },
            items: <String>['AVR', 'H755']
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Column(children: [
          ...
          precisionMode(),
          OutlinedButton(
              onPressed: sockEvent_state
                  ? () async {
                      await sockEvent.write("SSD_");
                    }
                  : null,
          ...
          OutlinedButton(
              onPressed: sockEvent_state
                  ? () async {
                      await sockEvent.write("BSD_");
                    }
                  : null,
              child: const Text("charet")),
        ]),
      ),
    );
  }
